...
log "Installing Ansible and running playbooks"

sudo --login --non-interactive --user=vagrant -- bash -c "
  pip install ansible paramiko &&
  cd /vagrant/ansible &&
  ansible-galaxy install -r requirements.yml &&
  ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i inventory.yml site.yml &&
  ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i inventory.yml router-config.yml
"
